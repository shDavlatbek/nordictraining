...
function Forms_GoogleRecaptcha() {
  ...
  that.getToken = function () {
    try {
      grecaptcha.ready(function () {
        grecaptcha.execute("6Lck3r0ZAAAAAOFc__oZANv72nZ3K29O-qsOIYPp", { action: "users_forms_submit" }).then(function (token) {
    ...
    $("a").each(function () {
      ...
      if (hrefTXT.indexOf("http://") === 0 || hrefTXT.indexOf("https://") === 0) {
        ...
        if (!isAllowedDomainBOO) {
          var newUrl = window.location.protocol + "//" + websiteDomain + "/versions/2/include/external_redirect.php?websiteID=" + encodeURIComponent(websiteID) + "&url=" + encodeURIComponent(hrefTXT);
        ...
        $(".s123-icon-converter").each(function () {
          ...
          if ($this.closest("#top-menu-mobile").length > 0) {
            ...
            if ($this.attr("style").indexOf("display") != -1) {
              $newIcon.css({ display: $this.css("display") });
            }
...
S123.Jarallax = (function () {
  ...
  _.set = function ($jarallax) {
    ...
    var options = { speed: isMobileDevice.any() ? 0.75 : 0.5 };
    ...
    if ($jarallax.data("backgroundcolor")) {
      $jarallax.find(".jarallax-container").css({ backgroundColor: $jarallax.data("backgroundcolor") });
    }
    if ($jarallax.data("opacity")) {
      if ($jarallax.css("opacity") == 1) {
        $jarallax.find(".jarallax-container > div").css({ opacity: $jarallax.data("opacity") });
      }
    }
    if ($jarallax.data("j-filter")) {
      $jarallax.find(".jarallax-container > div").css({ filter: $jarallax.data("j-filter") });
    }
  };
...
function Parallax_active(active) {
  ...
  if (active) {
    ...
    $("html").addClass("parallax-active").removeClass("parallax-disabled");
  } else {
    if ($("html").hasClass("parallax-disabled")) return;
    $("html").removeClass("parallax-active").addClass("parallax-disabled");
...
function TopSectionInitialize() {
  $(document).on("s123.page.ready", function (event) {
    $.each(
      $("#websitePopupHomeVideo, .promoVideoPopup .iconsCircle, #home_buttonText, #home_buttonText_1, #topAction_buttonText_1, #topAction_buttonText_2, #promoRedirectButton1, #promoRedirectButton2"),
      function (index, el) {
...
function intializePopupVideoPlayers($el) {
  $el.off("click.intializePopupVideoPlayers").on("click.intializePopupVideoPlayers", function (e) {
    ...
    if (player === "site123") {
      videoURL =
        "/include/globalVideoPlayer.php?websiteID=" + $("#websiteID").val() + "&website_uniqueID=" + $("#website_uniqueID").val() + "&cad=1&url=" + encodeURIComponent(videoURL) + "&fluid=true";
    }
              ...
              $(".contactUsConfirm")
                ...
                .on("click", function () {
                  ...
                  $.ajax({
                    ...
                    data: { websiteid: websiteID, w: w, email: userEmail, firstName: userFirstName, lastName: userLastName, phone: userPhone, country: userCountry },
...
function OpenSearchWindow(closeLocation, customPlaceholder) {
  ...
  searchInput +=
    '<input type="text" name="widget-search-form-keyword" class="widget-search-form-keyword form-control input-lg" placeholder="' + placeholder + '" aria-required="true" autocomplete="off">';
...
function ModulesDataModelInitialize() {
  $(document).on("s123.page.ready.data-model", function (event) {
    $('a[data-rel="popupScreen"]')
      .off("click.popupScreen")
      .on("click.popupScreen", function (event) {
        event.preventDefault();
        var $this = $(this);
...
function GoToTopButtonInitialize() {
  $(document).on("s123.page.ready", function (event) {
    ...
    $(document)
      ...
      .on("S123MagicButton.initialized.GoToTopButton", function (event) {
        ...
        if (($("html").attr("dir") === "rtl" && !$(".all-magic-buttons").hasClass("mg-b-icon-position-right")) || $(".all-magic-buttons").hasClass("mg-b-icon-position-left")) {
          $gotoTop.css({ left: $allMagicButtons.width() + parseInt($allMagicButtons.css("left")) + 10, bottom: "25px", right: "auto" });
        } else {
          $gotoTop.css({ right: $allMagicButtons.width() + parseInt($allMagicButtons.css("right")) + 10, bottom: "25px", left: "auto" });
        }
        if (isFreePackage) {
...
function ContactUsMapObject() {
  $(document).on("s123.page.ready", function (event) {
    $(".s123-module-contact-map, .gmap-container")
      .find(".map-container")
      .each(function (index, upload) {
        var $this = $(this);
...
function ActivePopupActionButtonsInPage() {
  $('[data-toggle="search_menuCallActionIcons"]')
    .off("click")
    .click(function () {
      var $this = $(this);
      OpenSearchWindow($this.data("closeLocation"), websiteCustomSearchPlaceHolder);
    });
  $('[data-toggle="social_menuCallActionIcons"]')
    .off("click")
    .click(function () {
      var $this = $(this);
  ...
  $('[data-toggle="address_menuCallActionIcons"]')
    .off("click")
    .click(function () {
      var $this = $(this);
      ...
      submitHandler: function (form) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            $(".report-abuse-popup-thanks-box")
              .find(".report-abuse-close-btn")
              .on("click", function () {
  ...
  function fixSafariPromoImages() {
    if (IsWizard() && /^((?!chrome|android).)*safari/i.test(navigator.userAgent)) {
      $('.s123-module[data-module-type-num="1000"] .promoImageInline .lazyload:not(.loaded)').each(function (index, img) {
        LazyLoad.load(img, { callback_loaded: function (el) {} });
      });
    }
  }
}
...
function MoveFirstSection(sectionNUM) {
  ...
  $("html, body")
    .stop()
    .animate({ scrollTop: $pages.eq(sectionNUM).offset().top - offset }, 1250, "easeInOutExpo");
}
function MoveFirstSectionOrRedirect(url) {
  ...
  if ($pages.length !== 0) {
    $("html, body")
      .stop()
      .animate({ scrollTop: $pages.eq(0).offset().top - offset }, 1250, "easeInOutExpo");
  } else {
...
function ScrollToModule(fromModuleID, toModuleID) {
  ...
  if ($("html.inside_page").length > 0) {
    ...
    if ($scrollTo.length !== 0) {
      $("html, body")
        .stop()
        .animate({ scrollTop: $scrollTo.offset().top - offset }, 1250, "easeInOutExpo");
    }
  }
}
...
function activeDropDownMenusAction() {
  $(".dropdown-submenu > a")
    .off("click.activeDropDownMenusAction")
    .on("click.activeDropDownMenusAction", function (event) {
      if ($(this).parent().data("is-eCommerce")) {
        event.preventDefault();
      }
    });
  $(".navPages li").find("a").off("mouseenter.hideHoverMenu");
  $(".navPages, #top-menu .navActions")
  ...
  $(".navPages > li")
    ...
    .find(" > a")
    .off("mouseenter.hideHoverMenu")
    .on("mouseenter.hideHoverMenu", function (e) {
  ...
  if (($place == "footer" || findBootstrapEnvironment() != "xs") && CheckMenuWidthSpace($place) && $nav.find(">li>a").length > 1) {
    if (CheckMenuWidthSpace($place)) {
      if ($nav.find(".extra-nav-more").length == 0) {
        var x =
          '<li class="moduleMenu extra-nav-more dropdown-submenu"><a aria-haspopup="true" aria-expanded="true" onclick="return false;"><span class="txt-container">' +
...
function CheckMenuWidthSpaceHeight() {
  switch ($("#layoutNUM").val()) {
    default:
      if (
        $(window).outerHeight(true) - 20 <
        $("#header .header-column-logo").outerHeight(true) + $("#header .header-column-menu").outerHeight(true) + $("#header .header-column-menu-actions").outerHeight(true)
      ) {
        return true;
      } else {
        return false;
...
function ShowMenuAfterReduseSize($place) {
  ...
  if ($("#top-menu").length > 0 && $("#layoutNUM").val() == "21" && $place == "header") {
    ...
    if (
      $("#top-menu .navPages > li")
        .eq(saveLIplace - 1)
        .outerWidth(true) *
        0.6 <=
      extraPaddingFromSideOne
    ) {
  ...
  if ($("#header").length == 0 && $("#top-menu").length > 0 && $place == "header") {
    ...
    var $replaceActionButtonsToIcon =
      $(".replaceActionButtonsToIcon").length > 0
        ? $(".replaceActionButtonsToIcon")
        : $(`<li class="header-menu-wrapper replaceActionButtonsToIcon dropdown-submenu">
...
<ul class="header-action-btns-items"></ul>
    ...
    if (
      $("#mainNavMobile").is(":visible") == false &&
      rectMenu > rectHeader &&
      $("#top-menu .navActions .header-menu-wrapper").length > 0 &&
      $(".replaceActionButtonsToIcon").length > 0 &&
      $(".replaceActionButtonsToIconRemoveExtra").length == 0 &&
      $("#top-menu.affix").length == 0
    ) {
      $replaceActionButtonsToIcon
        .find(".site-dropdown-menu .header-icons-container .header-icons-items")
        .prepend($("#top-menu").find(".header-phone-wrapper, .header-address-wrapper, .header-social-wrapper, .header-search-wrapper, .header-email-wrapper"));
  ...
  if ($place == "" || $place == "header") {
    $("#mainNav #top-menu .navPages, #mainNav #top-menu .navActions, #mainNav #top-menu .headerSocial, #header .header-row").css({ opacity: "1" });
  }
  if ($place == "footer") {
    $(".global_footer .nav").css({ opacity: "1" });
  }
    ...
    setTimeout(function () {
      ...
      if ($("html").attr("dir") != "rtl") {
        if (rect.right > window.innerWidth && rect.width < window.innerWidth) {
          $dropdown.css({ left: "auto", right: isCategory ? "98%" : "0" });
        }
      } else {
        if (rect.left < 0 && rect.width < window.innerWidth) {
          $dropdown.css({ right: "auto", left: isCategory ? "98%" : "0" });
        }
      }
...
function PageScrollByClick() {
  $(document).on("s123.page.ready.pageScrollByClick", function (event) {
    var offset = findBootstrapEnvironment() != "xs" ? menuScrollOffset : menuScrollOffset_mobile;
    $("a.page-scroll")
      .off("click.scrollEvent")
      .on("click.scrollEvent", function (event) {
        var $anchor = $(this);
        var scroll_val = $($anchor.attr("href")).offset().top - offset;
        if (scroll_val < 0) scroll_val = 0;
        $anchor.blur();
        $("html, body").stop().animate({ scrollTop: scroll_val }, 1250, "easeInOutExpo");
        event.preventDefault();
      });
  });
}
...
function findBootstrapColPerRow($sectionItemsContainer) {
  ...
  $sectionItemsContainer.children().css({ paddingLeft: item_spacing / 2, paddingRight: item_spacing / 2 });
  return slidesToShow;
}
...
S123.VideoHandler = (function () {
  ...
  _.init = function () {
    ...
    if (isMobileDevice.any()) {
      $('.s123-video-handler:not([data-player="youtube"]):not([data-is-full-screen="true"])')
        .imagesLoaded()
...
function s123VideoHandler($obj) {
  ...
  if (player === "site123") {
    videoURL =
      "/include/globalVideoPlayer.php?websiteID=" +
      $("#websiteID").val() +
      "&website_uniqueID=" +
      $("#website_uniqueID").val() +
      "&cad=1&url=" +
  ...
  var $iframe = $(
    '<div class="video-wrapper"><iframe id="v-h-' +
      video_id +
      '" data-player="' +
      player +
      '" style="' +
      ...
      videoURL +
      '" allow="autoplay; fullscreen" frameborder="0" allowfullscreen></iframe></div>'
  );
  ...
  (function () {
    ...
    function YouTubePlayerInitialize(video_id) {
      youtube_player = new YT.Player("v-h-" + video_id, { events: { onReady: onPlayerReady } });
...
function s123EditorVideoTagsHandler() {
  $(".fr-view video").each(function () {
    ...
    $video.replaceWith(
      '<div class="s123-video-handler" data-player="site123" data-video="' +
        src +
        '" style="max-width: 100%;max-height:100%;"><img style="' +
        $video.attr("style") +
        '" src="' +
        thumbnail +
        '"><div class="s123-video-cover"><a class="s123-video-play-icon"><i class="fa fa-play"></i></a></div></div>'
    );
  });
}
...
var ClientZone = (function () {
  ...
  CZ.getClientAddress = function (callback) {
    ...
    $.ajax({
      ...
      data: { websiteID: $("#websiteID").val(), w: $("#w").val() },
      success: function (response) {
...
function generateSharingPopoverHTML(popOverTitle, url, title) {
  ...
  html +=
    '<li style="margin-right: 5px;"><a class="btn" href="https://www.facebook.com/sharer/sharer.php?u=' +
    url +
    "&t=" +
    title +
    '" title="Share on Facebook" target="_blank">' +
    S123.s123IconToSvg.getHtml("facebook", "", "") +
    "</a></li>";
  html +=
    '<li style="margin-right: 5px;"><a class="btn" href="https://twitter.com/intent/tweet?source=' +
    url +
    "&text=" +
    title +
    ":%20" +
    ...
    S123.s123IconToSvg.getHtml("twitter", "", "") +
    "</a></li>";
...
function sharePopover($button, html) {
  ...
  $sharingURL.click(function () {
    $.gritter.add({ title: translations.linkCopiedToClipboard, class_name: "gritter-success", time: 6000 });
  });
  $button.popover({
    container: "body",
    html: "true",
...
$.fn.containerTextWidth_site123 = function (fontSize) {
  ...
  var html_calc =
    '<div id="containerTextWidth_site123" style="position:absolute;opacity:0;width:' +
    bodyWidth +
    "px;display:table;font-size:" +
    fontSize +
    ";letter-spacing: " +
...
var s123MobileMenu = new (function () {
  ...
  that.init = function (settings) {
    $(document).on("s123.page.ready", function (event) {
      ...
      that.animation = 400;
      that.isOpened = false;
      $(".header-menu-wrapper .mobile-menu-btn")
        .off("click")
        .click(function () {
          var $this = $(this);
      ...
      topWindow
        .$(topWindow.document)
        .off("mobileMenuColorCorrection")
        .on("mobileMenuColorCorrection", function (event, val) {
  ...
  function generateHTML() {
    if (IsIE11()) {
      that.$source
        .find(".dropdown-submenu a > span:not(.txt-container)")
  ...
  function screenResizeHandler() {
    $(window)
      .off("resize.mobile_menu")
      .on("resize.mobile_menu", function () {
        that.contentOffset = $(window).width();
        ...
        if (that.isOpened) {
          ...
          if (that.isRtl) {
            that.$ul.css({ right: that.contentOffset * -1 });
          } else {
            that.$ul.css({ left: that.contentOffset * -1 });
          }
        }
      });
  }
...
function s123InfiniteScroll(settings) {
  ...
  that.addLoadNextPageAbility = function () {
    $(window)
      .off("scroll.s123InfiniteScroll" + that.id)
      .on("scroll.s123InfiniteScroll" + that.id, function () {
...
S123.globalContactEmail = (function () {
  ...
  that.buildEmailToolForm = function () {
    ...
    if (that.settings.useCustomForm == "on" && that.custom_form_html.length !== 0) {
      ...
      html +=
        '<input type="text" name="emailForm_fullName" placeholder="' +
        translations.globalContactEmail.fullName +
        '" class="form-control" required data-msg-required="' +
        translations.jqueryValidMsgRequire +
        '">';
      html += "</div>";
      html += "</div>";
      html += '<div class="col-sm-6 col-xs-12">';
      ...
      html +=
        '<input type="text" name="emailForm_email" placeholder="' +
  ...
  that.initMultiStepsButtons = function () {
    ...
    that.$nextButton.off("click").on("click", function () {
      ...
      if (that.$form.offset().top - offset < $(window).scrollTop()) {
        $("html, body")
          .stop()
          .animate({ scrollTop: that.$form.offset().top - offset }, 1250, "easeInOutExpo");
      }
    });
  };
  that.reset = function () {
...
function HomepageCountdown() {
  $(document).on("s123.page.ready", function (event) {
    ...
    countdownWidget.init({
      ...
      translate: { days: $countdown.data("days"), hours: $countdown.data("hours"), minutes: $countdown.data("minutes"), seconds: $countdown.data("seconds") },
      onStop: function () {
...
function S123Header_InitializeActionButtons(isTriggerPageReady = true) {
  ...
  var btns = {
    mobile: { $el: $("#mainNavMobile .navActions"), list: ["phone", "email", "address", "social", "search", "cart", "wishList", "clientZone"] },
    desktop: {
  ...
  $.each(btns, function (type, data) {
    ...
    if (type == "mobile") {
      ...
      if (settings.onepage == "2" && (btns > 1 || icons > 3 || btns + icons > 3)) {
        var $landingPageBars = $(
          '<li class="header-menu-wrapper landingPageBars dropdown-submenu"><a data-close-location="left" class="btn l-pagebars-btn" data-is-mobile="true" role="button" data-container="body" data-toggle="menuCallActionIcons">' +
            S123.s123IconToSvg.getHtml("bars", "", "") +
            '</a><ul class="site-dropdown-menu navActions-dropdown" data-rel="navActions" style="opacity: 1;"></ul></li>'
        );
        var $btns = $("<div></div>").append($html);
        var icons = $btns.find(".header-icon-wrapper").length;
        var btns = $btns.find(".header-btn-wrapper").length;
  ...
  function getButtonHTML(btn, type, settings) {
    ...
    switch (btn) {
      ...
      case "address":
        ...
        html +=
          '<a data-close-location="right" class="actionButton" role="button" data-container="body" data-toggle="address_menuCallActionIcons">' +
          S123.s123IconToSvg.getHtml("location-arrow", "", "") +
          "</a>";
        html += "</li>";
        break;
      case "social":
        ...
        html +=
          ...
          S123.s123IconToSvg.getHtml("share-alt", "", "") +
          "</a>";
        html += "</li>";
        break;
      case "search":
        ...
        html +=
          '<a data-close-location="right" class="actionButton" role="button" data-container="body" data-toggle="search_menuCallActionIcons">' + S123.s123IconToSvg.getHtml("search", "", "") + "</a>";
        html += "</li>";
        break;
      case "wishList":
      ...
      case "clientZone":
        ...
        html +=
          '<a class="client-zone-link" data-close-location="right" href="' +
          globalLanguageChildLan +
          "/" +
          "?clientZone=1" +
          settings.btns[btn].isManageURL +
          '">' +
          S123.s123IconToSvg.getHtml("user", "", "") +
          "</a>";
        html += "</li>";
        break;
      case "cart":
        if (settings.isSideMenu) {
          html +=
            '<li class="header-cart-wrapper orderOpenCart header-icon-wrapper' +
            (settings.btns[btn].isActive ? " show-static" : "") +
            ($("#websiteHeaderSettings").data("arranged-icons") === true ? " arranged-icons" : "") +
            " " +
            (settings.isMenuMainColor ? "parent-main-color-handler" : "") +
            '" data-has-online-store="' +
            settings.btns[btn].hasOnlineStore +
            '"><div class="s-c-w">' +
            settings.btns[btn].showMenuActionButtons +
            "</div></li>";
        } else {
          html +=
            '<li class="header-cart-wrapper orderOpenCart' +
            (settings.btns[btn].isActive ? " show-static" : "") +
            ($("#websiteHeaderSettings").data("arranged-icons") === true ? " arranged-icons" : "") +
            " " +
            (settings.isMenuMainColor ? "parent-main-color-handler" : "") +
        ...
        if (type == "mobile") {
          ...
          if (settings.btns[btn].phoneLinkIcon.icon.indexOf(".png") === -1) {
            html +=
              ...
              S123.s123IconToSvg.getHtml(settings.btns[btn].phoneLinkIcon.icon, settings.btns[btn].phoneLinkIcon.icon, "") +
              "</a>";
          } else {
            html +=
              '<a data-close-location="right" class="actionButton" role="button" data-container="body" data-toggle="phone_menuCallActionIcons" data-icon-type="socialImage"><img src="' +
              $GLOBALS["cdn-system-files"] +
              "/files/icons/socialNetworksBrands/" +
              settings.btns[btn].phoneLinkIcon.icon +
              "?v=" +
              $GLOBALS["v-cache"] +
              '" alt="' +
              settings.btns[btn].phoneLinkIcon.alt +
              '"></a>';
          }
          html += "</li>";
        } else {
          ...
          if (settings.btns[btn].headerPhoneNumber_style == "1") {
            if (settings.btns[btn].phoneLinkIcon.icon.indexOf(".png") === -1) {
              ...
              html +=
                '<a data-close-location="right" class="actionButton" role="button" data-container="body" data-toggle="phone_menuCallActionIcons" data-social-icon="' +
                settings.btns[btn].phoneLinkIcon.icon.replace(".png", "") +
                '">';
              html +=
                '<img src="' +
                $GLOBALS["cdn-system-files"] +
...
S123.ElementSizeChangeManager = (function () {
  ...
  _.escm_observe = function (id, changesToObserve) {
    ...
    _.observing[id] = { id: id, $el: $element, changesToObserve: changesToObserve };
...
var site123FixText = (function () {
  ...
  _.fixText = function ($textEle, $parentBox, compressor) {
    ...
    let fixText = { isFitText: false, minFontSize: fontSize, maxFontSize: fontSize };
  ...
  _.getMaxFontSize = function ($textEle, $parentBox, compressor) {
    ...
    return { isfitText: isfitText, maxFontSize: Math.min(...maxFontSizeArr) };
  };
