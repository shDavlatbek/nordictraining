          ...
          $sectionThis.magnificPopup({
              ...
              callbacks: {
                  ...
                  open: function(item) {
                      ...
                      $(document).trigger('s123.page.ready.ReplaceExternalLinksInFreeSubDomain');
                  },
                  change: function() {
                      if (this.isOpen) {
...
function GalleryModuleInitialize_Layout6() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          ...
          var $categoriesDescription = $sectionThis.find('.gallery-category-description');
          ...
          var $galleryItem = $sectionThis.find('.gallery-image');
          var isScrolling;
          $sectionThis.find('.gallery-container').prepend($sectionThis.find('.gallery-category-description'));
          ...
          if (layout_customize && layout_customize.display_type === 'alwaysRun') {
              flickitySettings.friction = 0.28;
              flickitySettings.autoPlay = 1500;
              flickitySettings.freeScroll = false;
              flickitySettings.prevNextButtons = false;
              flickitySettings.pageDots = false;
              flickitySettings.pauseAutoPlayOnHover = false;
              flickitySettings.wrapAround = true;
          }
          ...
          if (layout_customize && layout_customize.display_type === 'alwaysRun') {
              $flickityContainer.on('dragStart.flickity', function(event, pointer) {
                  clearTimeout(isScrolling);
              });
              $flickityContainer.on('dragEnd.flickity', function(event, pointer) {
                  clearTimeout(isScrolling);
                  var $this = $(this);
          ...
          $categories.off('click').on('click', function() {
              ...
              $this.addClass('active');
              $categoriesDescription.removeClass('active-description');
              $sectionThis.find('.gallery-category-description[data-filter="' + $this.data('filter') + '"]').addClass('active-description');
              $flickityContainer.flickity('resize');
              return false;
          });
...
function GalleryModuleInitialize_Layout7() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          ...
          if ($isotopeContainer.hasClass('carousel-style-2')) {
              flickitySettings.lazyLoad = 4;
              flickitySettings.initialIndex = 3;
          }
          $flickityContainer.flickity(flickitySettings);
          $isotopeContainer.isotope({
              itemSelector: '.gallery-images-container, .gallery-image-caption'
          ...
          $flickityContainer.on('settle.flickity', function() {
              ...
              var caption = Gallery_getImageMagnificPopupCaption($(flkty.selectedElement).data('tool-title'), $(flkty.selectedElement).data('image-description'), $(flkty.selectedElement).data('image-link'), $(flkty.selectedElement).data('image-link-text'), $(flkty.selectedElement).data('image-page-url'), $(flkty.selectedElement).attr('title'), false, '', []);
              $this.next('.gallery-image-caption').html(caption);
              $(flkty.selectedElement).attr('alt', $(flkty.selectedElement).data('alt'));
          ...
          if ($categoriesDescription.length > 0) {
              $flickityContainer.addClass('gallery-images-container-with-description');
          }
          $categories.off('click').on('click', function() {
              var $this = $(this);
              $isotopeContainer.isotope({
...
jQuery(function($) {
  GalleryModuleInitialize_Layout9_10();
});
function GalleryModuleInitialize_Layout9_10() {
  $(document).on('s123.page.ready', function(event) {
      var $section = $('section.s123-module-gallery.layout-9:not(.s123-module-videos),section.s123-module-gallery.layout-10:not(.s123-module-videos)');
      $section.each(function(index) {
          var $sectionThis = $(this);
          var $categoriesFilter = $sectionThis.find('.filter li');
          var $categories = $sectionThis.find('.gallery-category');
          ...
          $categoriesFilter.off('click.gallery.first.cat.click').on('click.gallery.first.cat.click', function(event, pageLoad) {
              event.preventDefault();
              var $this = $(this);
              var filter = $this.data('filter');
              var $hideCategories = $categories.filter(':not([data-filter="' + filter + '"]):visible');
          ...
          if (!IsHomepage() && !IsRichPage()) {
              ...
              $categories.off('infiniteScroll.loaded').on('infiniteScroll.loaded', function(event, data) {
                  var $this = $(this);
                  var $items = $(data.itemsHtml);
                  if ($sectionThis.hasClass('layout-10')) {
                      $this.find('.grid .grid-item').addClass('grid-item-grow');
                      $this.find('.grid').append($items);
                  } else {
                      $this.append($items);
                  }
                  window.myLazyLoad.update();
          ...
          $firstCategory.trigger('click.gallery.first.cat.click', [true]);
          if ($sectionThis.data('prevent-download-image') && $sectionThis.data('prevent-download-image') == 'on') {
              $.each($galleryItem, function(index, image) {
                  image.oncontextmenu = function() {
                      return false;
...
function ServicesModuleInitialize() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          ...
          if ($sectionThis.hasClass('layout-10')) {
              $sectionThis.find('.service-item-description').each(function() {
                  const $paragraph = $(this);
                  const lineHeight = parseFloat($paragraph.css('font-size')) * 1.5;
                  const maxLines = 4;
                  const actualHeight = $paragraph.height();
                  const numberOfLines = actualHeight / lineHeight;
                  if (numberOfLines > maxLines) {
                      $paragraph.addClass('show-more');
                      $paragraph.attr('data-rel', 'tooltip');
                      $paragraph.css('height', lineHeight * maxLines);
...
function ServicesModuleMobileViewCarouselInit($section) {
  let sectionID = $section.get(0).id.replace('section-', '');
  let $sectionItemsContainer = $section.find('.service-grid');
  let mobileViewTypeNUM = $section.data('mobile-view-type');
  ServicesModuleMobileViewCarouselInitResizeHandler();
  if ($sectionItemsContainer.hasClass('slick-initialized')) {
      $sectionItemsContainer.css('visibility', 'none');
      $sectionItemsContainer.slick('unslick');
      $sectionItemsContainer.parent().find('.custom-arrow-container').remove();
  }
  if (findBootstrapEnvironment() != 'xs' || mobileViewTypeNUM != '2') {
      $sectionItemsContainer.css('visibility', 'visible');
      return;
  }
  $sectionItemsContainer.on('init', function(event) {
      $sectionItemsContainer.css('visibility', 'visible');
  });
  ...
  function ServicesModuleMobileViewCarouselInitResizeHandler() {
      $(window).off('resize.ServicesModuleMobileViewCarouselInit').on('resize.ServicesModuleMobileViewCarouselInit', function(event) {
          clearTimeout(window.ServicesModuleMobileViewCarouselInit_Delay);
          window.ServicesModuleMobileViewCarouselInit_Delay = setTimeout(function() {
              var $sections = $('section.s123-module-services');
              $sections.each(function(index) {
                  ServicesModuleMobileViewCarouselInit($(this));
              });
          }, 50);
      });
  }
...
function TeamModuleInitialize() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          ...
          if ($sectionThis.hasClass('layout-13')) {
              $sectionThis.find('.team-item-description').each(function() {
                  if (isMobileDevice.any())
                      return;
                  const $paragraph = $(this);
                  const lineHeight = parseFloat($paragraph.css('font-size')) * 1.5;
                  ...
                  if (numberOfLines > maxLines) {
                      ...
                      $paragraph.attr('data-rel', 'tooltip');
                      $paragraph.css('height', lineHeight * maxLines);
                      $paragraph.on('click.layout.13.open.more', function() {
                          var _this = $(this);
                          bootbox.alert({
                              className: 'item-show-more-popup',
                              title: _this.data('title'),
                              message: `<p class="item-show-more-text">${_this.html()}</p>`,
                              backdrop: true
                          });
                      });
                  }
...
function TeamModuleInitialize_Layout9() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          var $sectionThis = $(this);
          $section.off('module_layout_categories.shown').on('module_layout_categories.shown', function(event, catID) {
              var $category = $(this).find('.team-category[data-categories-filter="' + catID + '"]');
              var numberImagesInRow = getSlidesPerView($category);
              var spaceBetween = 30;
              var highestMember = 0;
              if (numberImagesInRow == 0) {
                  $category.find('.owl-carousel').addClass('manually-loaded');
                  $category.find('.team-member-wrap').css({
                      height: ''
                  });
                  initializeTeamPhone($sectionThis);
                  return;
              }
              ...
              var $teamMemberWrap = $category.find('.team-member-wrap').first();
              if ($teamMemberWrap.find('.team-member').length > 0) {
                  $category.find('.s123-module-shadows').css({
                      height: $teamMemberWrap.height() + 'px'
                  });
              }
              $category.find('.owl-carousel').owlCarousel({
                  items: numberImagesInRow,
                  autoplay: false,
                  autoplayTimeout: 0,
                  center: true,
                  dots: $category.find('.owl-carousel').data('item-count') > 7 ? false : true,
                  dotsEach: (numberImagesInRow % 2 == 1 ? true : false),
                  margin: spaceBetween,
                  loop: true,
                  lazyLoad: true,
                  stagePadding: 50,
          ...
          if ($sectionThis.find('.team-category[data-categories-filter="s123-g-show-all"]').length > 0) {
              $sectionThis.trigger('module_layout_categories.shown', ['s123-g-show-all']);
          }
          var categories = new ModuleLayoutCategories({
              $items: $sectionThis.find('.team-category'),
              $categoriesContainer: $sectionThis.find('.categories-panel'),
      ...
      function initializeTeamPhone($teamSection) {
          $teamSection.off('click.initializeTeamPhone').on('click.initializeTeamPhone', '.team-phone-btn', function() {
              var $teamPhone = $(this).closest('.team-phone');
              buildPopup('teamPopupFloatDivPhone', '', $teamPhone.find('.team-phone-popover').html(), '', true, true, true, '', '');
          })
      }
  ...
  function getSlidesPerView($category) {
      var slidesPerView = 3.3;
      var originalItems = [];
      $category.find('.team-member').each(function() {
      ...
      case 'xs':
      case 'sm':
          slidesPerView = originalItems.length < 2 ? 0 : 1;
          break;
      default:
          slidesPerView = originalItems.length <= 3 ? 0 : 3;
          break;
      }
  ...
  if (IsWizard()) {
      $(document).on('wizard.preview.device.changed', function(event) {
          ...
          $section.each(function(index) {
              ...
              var $carousel = $category.find('.owl-carousel');
              $carousel.trigger('destroy.owl.carousel');
              $carousel.removeClass('owl-loaded owl-carousel-init manually-loaded');
              $sectionThis.trigger('module_layout_categories.shown', [$category.data('categories-filter')]);
          });
      });
  }
}
jQuery(function($) {
  TeamModuleInitialize_Layout14();
});
function TeamModuleInitialize_Layout14() {
  $(document).on('s123.page.ready', function(event) {
      var $section = $('section.s123-module-team.layout-14');
      $section.each(function(index) {
          var $sectionThis = $(this);
          $sectionThis.find('.team-category').each(function() {
              $(this).find('.team-member').first().addClass('active');
              $(this).find('.team-image').first().addClass('active');
              $(this).find('.team-memeber-info').first().removeClass('team-more-info-hidden');
          });
          $sectionThis.find('.team-members .team-member').on('click mouseenter', function() {
              let $this = $(this);
              if ($this.hasClass('active'))
                  return;
              if (!$sectionThis.find('.team-image.active').is(':animated')) {
                  let imgKey = $this.data('img-key');
                  $sectionThis.find('.team-member').removeClass('active');
                  $sectionThis.find('.team-image.active').fadeOut(500).removeClass('active');
                  $sectionThis.find('.team-memeber-info').addClass('team-more-info-hidden');
                  $sectionThis.find('.team-member[data-img-key="' + imgKey + '"]').addClass('active');
                  $sectionThis.find('.team-image[data-img-key="' + imgKey + '"]').fadeIn(500).addClass('active');
                  $sectionThis.find('.team-memeber-info[data-img-key="' + imgKey + '"]').removeClass('team-more-info-hidden');
              }
          });
          $sectionThis.off('module_layout_categories.shown').on('module_layout_categories.shown', function(event, catID) {
              $category = $(this).find('.team-category[data-categories-filter="' + catID + '"]');
              $sectionThis.find('.team-member').removeClass('active');
              $sectionThis.find('.team-image.active').fadeOut(500).removeClass('active');
              $sectionThis.find('.team-image').removeClass('active');
              $sectionThis.find('.team-memeber-info').addClass('team-more-info-hidden');
              let imgKey = $category.find('.team-member:first-child').data('img-key');
              $sectionThis.find(`.team-member[data-img-key="${imgKey}"]`).addClass('active');
              $sectionThis.find(`.team-image[data-img-key="${imgKey}"]`).fadeIn(500).addClass('active');
              $sectionThis.find(`.team-memeber-info[data-img-key="${imgKey}"]`).removeClass('team-more-info-hidden');
          });
...
function Comments_Initialize(settings) {
  ...
  that.customLabels = settings.customLabels ? settings.customLabels : {};
  ...
  that.submitHandler = function($form) {
      ...
      $form.off('submit.commentsForm').off('submit.commentsForm').on('submit.commentsForm', function(event) {
          ...
          $form.find('.blogSubmitButton').html(S123.s123IconToSvg.getHtml('spinner', 'fa-spin', ''));
          $.ajax({
              type: "POST",
              ...
              success: function(data) {
                  ...
                  if (that.onFormSubmit)
                      that.onFormSubmit.call(this, dataObj.newCommentID, dataObj.blockComment);
              }
          });
          return false;
      });
  ...
  that.showReplyModal = function(t, childForm) {
      ...
      x += '<textarea class="form-control comment_message" name="comment_message" placeholder="' + (that.customLabels.commentResponseText ? that.customLabels.commentResponseText : t.data('tran-comment')) + '" style="min-height: 150px;" required data-msg-required="' + t.data('tran-this-field-is-required') + '"></textarea>';
      x += '</div>';
      x += '<div class="row">';
      x += '<div class="col-xs-12 col-sm-6">';
      x += '<div class="form-group">';
      x += '<input type="text" name="contact_name" placeholder="' + (that.customLabels.commentName ? that.customLabels.commentName : t.data('tran-name')) + '" class="form-control" required data-msg-required="' + t.data('tran-this-field-is-required') + '">';
      x += '</div>';
      x += '</div>';
      x += '<div class="col-xs-12 col-sm-6">';
      x += '<div class="form-group">';
      x += '<input type="text" name="contact_email" placeholder="' + (that.customLabels.commentEmail ? that.customLabels.commentEmail : t.data('tran-email-address')) + '" class="form-control" required data-msg-required="' + t.data('tran-this-field-is-required') + '" data-rule-email="true" data-msg-email="' + t.data('tran-please-enter-valid-email') + '">';
      x += '</div>';
      x += '</div>';
      x += '</div>';
      x += '<div class="text-center">';
      x += '<button type="submit" class="btn btn-primary blogSubmitButton" data-text="' + t.data('tran-post-reply') + '">' + (that.customLabels.replyButtonText ? that.customLabels.replyButtonText : t.data('tran-post-reply')) + '</button>';
      x += '</div>';
      x += '<div class="text-center comments-message-text">';
      x += '<small>* ' + (that.customLabels.commentMessage ? that.customLabels.commentMessage : t.data('tran-the-email-will-not-be-published-on-the-website')) + '</small>';
      x += '</div>';
      x += '</div>';
  ...
  Rating = new function() {
      ...
      R.init = function(settings) {
          ...
          R.$raty.raty({
              ...
              number: 5,
              hints: R.getHints(),
              starHalf: (IsIE11() ? 'fa fa-star-half star-half' : 'star-half svg-m'),
              starOff: (IsIE11() ? 'fa fa-star-o star-o' : 'star-o svg-m'),
              starOn: (IsIE11() ? 'fa fa-star star' : 'star svg-m'),
  ...
  function addCommentsToPage(commentsHtml) {
      ...
      $commentsContainer.find('.blog-reply-to').each(function(index, replyLink) {
          $(replyLink).off('click').on('click', function() {
              ...
              if ($childForm.hasClass('hidden')) {
                  ...
                  $('.blogReplyForm').addClass('hidden');
                  $('.blog-reply-to').html($(this).data('text-reply'));
      ...
      if (hasRating) {
          if (that.onLoad)
              that.onLoad.call(this, $commentsContainer.find('.commentBox').length, $commentsContainer.find('#reviewAvg').val(), false);
      }
  }
  that.init();
}
...
function CustomFormModuleInitialize() {
  $(document).on('s123.page.ready', function(event) {
      ...
      $section.each(function(index) {
          var $sectionThis = $(this);
          initializeUniqueSubmission($sectionThis);
                  ...
                  submitHandler: function(form) {
                      ...
                      $.ajax({
                          ...
                          success: function(data) {
                              ...
                              if (clickAction == 'thankYouMessage' || clickAction == '') {
                                  $sendingDialog.on('hide.bs.modal', function(event) {
                                      initializeUniqueSubmission($sectionThis);
                                  });
                              }
                              WizardNotificationUpdate();
  ...
  function initializeUniqueSubmission($sectionThis) {
      if ($('#w').val().length != 0)
          return;
      $sectionThis.find('#uniqueSubmission').each(function(index) {
          var $uniqueSubmission = $(this);
          var websiteID = $('#websiteID').val();
          var uniqueSubmissionID = $uniqueSubmission.data('unique-id');
          var cookie = getCookie(websiteID + '-cfus-' + uniqueSubmissionID);
          if (cookie == '1') {
              $sectionThis.find('.customForm').hide();
              $uniqueSubmission.fadeIn(350);
          }
      });
  }
...
function portfolioModuleInitialize() {
  $(document).on('s123.page.ready', function(event) {
      var $section = $('section.s123-module-portfolio.layout-4');
      if ($section.length === 0)
          return;
      $section.each(function(index) {
          var $sectionThis = $(this);
...
jQuery(function($) {
  RichPageModuleInitialize();
});
function RichPageModuleInitialize() {
  $(document).on('s123.page.ready', function(event) {
      if (!IsWizard()) {
          RichPagePromosHandler.init();
      }
  });
  var RichPagePromosHandler = function() {
      var _ = {
          fixColors: false
      };
      _.init = function() {
          _.$section = $('section.s123-module-rich-page');
          if (_.$section.length === 0)
              return;
          _.$sections = _.$section.siblings();
          _.$sections.each(function(index, section) {
              var $thisSection = $(this);
              if ($thisSection.attr('class').indexOf('s123-promo-module') != -1) {
                  $thisSection.remove();
                  _.fixColors = true;
              } else if (!(whatScreen.any() == 'mobile' && $thisSection.hasClass('h-s-f-hm'))) {
                  return false;
              }
          });
          if (_.fixColors) {
      ...
      ;
      function fixModuleColors() {
          _.$sections.filter(':visible').each(function(index, module) {
              var $module = $(this);
              if (index % 2 == 0) {
                  $module.addClass('bg-primary');
              } else {
                  $module.removeClass('bg-primary');
              }
          });
      }
      return _;
...
function headersModuleInitialize_Layout() {
  $(document).on("s123.page.ready", function(event) {
      ...
      $sections.each(function(index) {
          ...
          $s.find('.header1, .header2').each(function(index) {
              ...
              if ($this.data('line-height') && $this.data('line-height').length > 0) {
                  $this.get(0).style.setProperty('--header_line_height', $this.data('line-height'));
          ...
          if ($s.hasClass('layout-20')) {
              ...
              if ($firstImage.length > 0) {
                  if ($firstImage.data('bg') !== undefined) {
                      ...
                      $firstImage.closest('.headers-img-wrap').addClass('circle-fixed-width');
                  }
              }
          }
          ...
          if ($s.hasClass('layout-26') && $s.attr('data-version') !== undefined && $s.attr('data-version') == '2') {
              if (IsWizard()) {
          ...
          function updateBackgroundColor() {
              ...
              var color = $s.hasClass('bg-primary') ? getComputedStyle(document.documentElement).getPropertyValue('--modules_color_box') : getComputedStyle(document.documentElement).getPropertyValue('--modules_color_second_box');
              ...
              case 'background-primary-color btn-primary-text-color':
                  color = $s.hasClass('bg-primary') ? getComputedStyle(document.documentElement).getPropertyValue('--modules_color_section_main') : getComputedStyle(document.documentElement).getPropertyValue('--modules_color_second_section_main');
                  break;
              }
      ...
      $sections.each(function(index) {
          ...
          $s.find('.header1, .header2').each(function(index) {
              var $this = $(this);
              if ($s.hasClass('layout-7') || $s.hasClass('layout-9') || $s.hasClass('layout-22') || $s.hasClass('layout-23') || $s.hasClass('layout-24') || $s.hasClass('layout-26') || $s.hasClass('layout-27') || $s.hasClass('layout-29') || $s.hasClass('layout-33')) {
          ...
          function fitHeaderTextToParentContainer($textEl) {
              ...
              if (!$.isNumeric(fontSize))
                  return;
              var $parentContainer = $textEl.closest('.headers-text-resize-container');
              if ($parentContainer.length > 0 && $parentContainer.width() < $textEl.width()) {
                  $textEl.css('visibility', 'hidden');
                  var parentContainerWidth = $parentContainer.width();
                  if ($textEl.closest('section').hasClass('layout-9') || $textEl.closest('section').hasClass('layout-27')) {
